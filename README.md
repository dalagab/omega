...
Community source intake is repository automation rather than a client-side crawler. The **Add a plugin source** issue form accepts public HTTPS PluginMaster feeds; `source-submissions.yml` validates them with the production manifest parser, a bounded response size, a bounded plugin count, and public-network redirect checks. A successfully validated issue submission is scraped immediately, persisted to `sources/community-sources.json` **disabled by default**, and queues the catalog builder; persistence is revalidated from a fresh checkout before anything is committed. Omega-managed missing-source follow-up issues also accept public GitHub source replies, but an override is persisted only after Sigmascope confirms that the repository contains the matching Dalamud plugin identity. The issue remains open until the subsequent Sigmascope scan successfully inspects that source, at which point reconciliation closes it automatically. Numeric SQLite variant IDs are never used as durable override identities.

Sigmascope treats distribution feeds and original source repositories as separate provenance layers. It derives public source candidates from `RepoUrl`, resolved and declared package URLs, GitHub raw/release/tree/blob forms, and explicit overrides; reads the downloaded plugin manifest to recover the artifact's own AssemblyVersion; then prefers matching version tags before mutable branch heads. Exact mirrored package hashes may inherit a successfully resolved original-source association for the same plugin identity. These are provenance statements—source discovered, identity/version/origin matched—not a claim that visible source reproducibly built the downloaded DLL; `sourceToBinaryVerified` remains false unless a stronger verification mechanism proves that relationship.
...
The default Developer View now browses the published `security-evidence-v2` branch directly over `raw.githubusercontent.com`: it initially fetches only the atomic root and plugin indexes, then fetches individual variant records and bounded evidence shards only when you open them. A small revision-scoped cache avoids repeat transfers, and the header checks for a newer Evidence Revision once per minute. Local `--evidence-v2` and historical SQLite modes remain available for unpublished/debug snapshots and parity work, while production automation continues to use `developer_view.py audit` against its disposable working projection and small marketplace candidate before publication. See [`tools/security/README.md`](tools/security/README.md) for details.
