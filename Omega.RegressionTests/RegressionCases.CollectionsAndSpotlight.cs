...
internal static partial class RegressionCases
{
    ...
    internal static void TestDiscoverRiskOwnershipAndAvailabilityContract()
    {
        ...
        DoesNotContain(discover, "installed ? 44f : 12f", "installed list entries must never shift artwork to make room for the check mark");
        DoesNotContain(discover, "var artworkX = installed ?", "installed rich cards must never shift artwork to make room for the check mark");
        ...
        Contains(discover, "draw.AddCircleFilled(center", "installed state is rendered as a compact green circle over the artwork");
        Contains(discover, "draw.AddLine(a, b", "installed state draws the first stroke of a geometric check mark");
        Contains(discover, "draw.AddLine(b, c", "installed state draws the second stroke of a geometric check mark");
