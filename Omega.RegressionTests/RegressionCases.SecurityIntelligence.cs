...
internal static partial class RegressionCases
{
    internal static void TestPluginSecurityIntelligenceContract()
    {
        ...
        Contains(normalizedWorkflow, "cron: \"47 3,15 * * *\"", "Sigmascope has twice-daily independent safety-net runs");
        Contains(normalizedWorkflow, "workflows:\n      - \"Omega SQLite catalog builder\"", "Sigmascope runs after successful catalog builds");
        Contains(normalizedWorkflow, "tools/catalog/sigmascope.py", "Sigmascope code changes trigger immediate repository-side validation");
        ...
        Contains(normalizedWorkflow, "actions: read", "Sigmascope can read the exact upstream catalog artifact");
        Contains(normalizedWorkflow, "contents: write", "final validated v2 snapshot publication has repository write permission");
        Contains(workflow, "ref: security-evidence-v2", "Sigmascope begins from the last-known-good v2 evidence snapshot");
        Contains(workflow, "production_sigmascope_v2_pipeline.py", "Sigmascope analysis is staged through the production v2 orchestrator");
    ...
    internal static void TestCatalogCompactionWorkflowContract()
    {
        ...
        Contains(sigmascopeWorkflow, "Publish validated Security Evidence v2 snapshot atomically", "production Sigmascope publishes the detailed v2 snapshot only after validation");
        Contains(sigmascopeWorkflow, "Publish small client marketplace only after all v2 gates pass", "client Definitions publication follows v2 validation/audit gates");
        Contains(sigmascopeWorkflow, "omega-marketplace.sqlite.zip", "client release continues to use the small marketplace bundle");
        Contains(sigmascopeWorkflow, "security-developer-audit.json", "independent conclusion audit is a production publication gate");
