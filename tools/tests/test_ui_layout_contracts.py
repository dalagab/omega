...
class UiLayoutContractTests(unittest.TestCase):
    ...
    def test_repository_chooser_selection_and_provider_presentation_contract(self):
        ...
        self.assertIn("OpenInstallRepositoryRiskReview", install)
        self.assertIn("DrawInstallRiskReviewModal", install)
        self.assertIn("pendingInstallRiskAcknowledgementChecked", install)
        self.assertNotIn("OpenDalamudRepositoryRiskReviewFromInstall", install)
        ...
        self.assertIn("IsInstallSourceSelectable", details)
        self.assertIn("DescribeInstallUnavailability", details)
        self.assertIn("ResolveOrCreateInstallSource", install)
        self.assertIn("ImGuiWindowFlags.NoScrollbar", awareness)
        self.assertIn("SettingsSection.Repositories", sources)
        self.assertIn("ImGuiTableFlags.ScrollY", sources)
        self.assertIn("ImGui.TableSetupScrollFreeze(0, 1)", sources)
        self.assertIn("settings-tab-repositories", sources)
        self.assertIn("Blue entries are unmanaged local feeds", sources)
        self.assertIn("Add to Dalamud", sources)
        self.assertIn("ownedByOmega: false", sources)
        self.assertIn("IsRepositoryVisibleInSettings", sources)
        self.assertIn("plugin-source.yml", sources)
        self.assertNotIn("My Sources (", sources)
        self.assertIn('ImGui.TableSetupColumn("Installed"', sources)
        self.assertIn("GetInstalledPluginUsageByRepository", sources)
        self.assertIn("RemoveIfUnusedAsync", sources)
        self.assertIn("blockedByInstalledPlugins", sources)
        self.assertIn("plugin.Manifest.InstalledFromUrl", (ROOT / "Omega" / "Services" / "DalamudRepositoryBridge.cs").read_text(encoding="utf-8"))
        self.assertIn("Acknowledge risk", install)
    ...
    def test_marketplace_plugin_panels_use_semantic_ribbons_without_shifting_layout(self):
        ...
        self.assertIn("showListingRibbons: true", discover)
        self.assertIn("showListingRibbons: true", spotlight)
        self.assertIn("showListingRibbons: true", shelves)
        artwork = (ROOT / "Omega" / "UI" / "MarketplaceWindow.Artwork.cs").read_text(encoding="utf-8")
        self.assertIn("if (showListingRibbons && listingPanelMin is { } panelMin && listingPanelMax is { } panelMax)", artwork)
        self.assertIn("DrawPluginCardTopRibbons(plugin, installedPlugin, currentApi, panelMin, panelMax)", artwork)
        ...
        self.assertIn("FontAwesomeIcon.Question", ribbons)
        self.assertNotIn("shown as closed source", ribbons)
        ...
        self.assertIn("var leftX = panelMin.X + edgeInset", ribbons)
        self.assertIn("var rightX = panelMax.X - edgeInset - ribbonWidth", ribbons)
        self.assertIn("leftX += ribbonWidth + ribbonGap", ribbons)
        self.assertIn("rightX - ribbonWidth - ribbonGap", ribbons)
        self.assertIn("draw.PushClipRect(panelMin, panelMax, false)", ribbons)
        self.assertNotIn("ImGui.SetCursorPosY(contentStartY + Ui(26f))", spotlight)
        self.assertNotIn("ImGui.SetCursorPosY(Ui(34f))", shelves)
        self.assertNotIn("artworkMin.X", ribbons)
        self.assertIn("var glyphCenter = new Vector2(centerX, min.Y + (height * 0.5f))", ribbons)
        self.assertNotIn("DrawArtworkStatusFlag", ribbons)
        self.assertNotIn("RibbonGlyphOpticalOffset(icon)", ribbons)
        ...
        self.assertIn("draw.AddRectFilledMultiColor", ribbons)
        self.assertNotIn("installed ? 44f : 12f", discover)
        self.assertNotIn("var artworkX = installed ?", discover)


    def test_discover_rich_card_owns_its_card_bounds_without_shadowing_parameters(self):
        discover = (ROOT / "Omega" / "UI" / "MarketplaceWindow.Discover.cs").read_text(encoding="utf-8")
        signature = discover.split("private void DrawDiscoverRichCard(", 1)[1].split("    {", 1)[0]
        self.assertIn("float cardWidth)", signature)
        self.assertNotIn("Vector2 cardMin", signature)
        self.assertNotIn("Vector2 cardMax", signature)
        body = discover.split("private void DrawDiscoverRichCard(", 1)[1].split("private bool DrawDiscoverRichCardHeader(", 1)[0]
        self.assertIn("var cardMin = ImGui.GetWindowPos();", body)
        self.assertIn("var cardMax = cardMin + ImGui.GetWindowSize();", body)


    def test_filters_precede_page_identity_and_omega_update_notice_is_compact(self):
    ...
    def test_spotlight_security_matches_product_page_package_and_keeps_automation_separate(self):
        ...
        self.assertIn("showListingRibbons: true", shelves)
        self.assertIn("showListingRibbons: true", discover)
        self.assertIn("DrawPluginPanelUpdateState", spotlight)
        self.assertIn("DrawPluginPanelUpdateState", shelves)
        self.assertIn("DrawPluginPanelUpdateState", discover)
        ...
        self.assertIn("Review: {coverage}", security)
        self.assertIn("Artifact only", security)
        self.assertIn("not a probability", security)
    ...
    def test_about_keeps_identity_fixed_and_scrolls_lower_copy(self):
        ...
        self.assertIn("catalog.DatabaseSizeBytes", source)
        self.assertIn("FormatDefinitionsDatabaseSize", source)
        self.assertIn('ImGui.TextDisabled($"({FormatDefinitionsDatabaseSize(catalog.DatabaseSizeBytes)})")', source)
