...
setTimeout(()=>{
 ...
 view.innerHTML=`<div class=operations-diag-head><div><div class=eyebrow>GITHUB ACCESS · ACQUISITION</div><h1>Operations diagnostics</h1><p>Read-only capability, rate-budget and acquisition health. Opening this page uses the current snapshot; refresh performs bounded GitHub reads but never a mutating permission probe.</p></div><div class=operations-diag-actions><span id=operationsDiagSnapshot class="muted small">Snapshot not loaded</span><button id=operationsDiagRefresh>Refresh diagnostics</button></div></div><div id=operationsDiagSummary class=operations-diag-grid><div class=workspace-empty>Loading the current diagnostics snapshot…</div></div><section class=panel><div class=panelhead><div><h2>GitHub capabilities</h2><div class="muted small">Observed reads are separated from write capabilities that are configured but intentionally not tested by mutation.</div></div></div><div id=operationsDiagCapabilities class=operations-diag-capabilities><div class=workspace-empty>Capability observations have not been loaded yet.</div></div></section><section class=panel><div class=panelhead><div><h2>Permission & acquisition guidance</h2><div class="muted small">DeltaScope never returns the credential, and these diagnostics do not become Security Evidence.</div></div></div><ul id=operationsDiagGuidance class=operations-diag-guidance><li>Loading the current acquisition state…</li></ul></section>`;
 ...
 function renderFailure(message){last=null;$('operationsDiagSnapshot').textContent='Diagnostics unavailable';$('operationsDiagSummary').innerHTML=`<article class=operations-diag-card><h3>Diagnostics request</h3>${state('unavailable')}${kv([['Reason',message||'Unknown diagnostics error']])}</article>`;$('operationsDiagCapabilities').innerHTML='<div class=workspace-empty>Capabilities could not be projected from the current diagnostics request.</div>';$('operationsDiagGuidance').innerHTML=`<li>${esc(message||'Operations diagnostics are unavailable.')}</li>`}
 async function load(refresh=false){if(inFlight)return;inFlight=true;const button=$('operationsDiagRefresh');if(button)button.disabled=true;try{render(await api(`/api/operations/diagnostics${refresh?'?refresh=1':''}`))}catch(error){renderFailure(error.message);toniSay(`Operations diagnostics failed: ${error.message}`)}finally{inFlight=false;if(button)button.disabled=false}}
