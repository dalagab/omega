...
<section id="workbench-coverage" class="workspace-view" data-workbench-view="coverage"><div class="workspace-heading"><div><h1>Detection Coverage</h1><p>Inspect how completely SigmaScope can produce each retained observation family across the current plugin corpus. Historical risky versions remain archive evidence; this matrix is current-version coverage only.</p></div><span class="badge ro">SECURITY SYSTEM SELF-REVIEW · READ ONLY</span></div><div id="coverageCards" class="cards workspace-cards"></div><section class="panel coverage-workspace"><div class="panelhead"><div><h2>Observation coverage & blind spots</h2><div class="muted small">Coverage means the current variant has a complete analysis at the narrow producer revision that owns this observation family. Empty complete collections are valid negative evidence, not missing data.</div></div><button id="refreshCoverage">Refresh matrix</button></div><div class="detail"><div id="coverageMethod" class="coverage-method">Loading coverage method…</div><div class="coverage-toolbar"><input id="coverageSearch" placeholder="Filter collections, producer, dataset or rule…"><select id="coverageStatusFilter"><option value="">All states</option><option value="blind-spot">Blind spots</option><option value="degraded">Degraded</option><option value="attention">Needs attention</option><option value="healthy">Healthy</option></select><select id="coverageOriginFilter"><option value="">All origins</option><option value="artifact">Artifact</option><option value="artifact+source">Artifact + source</option><option value="source">Source</option><option value="catalog">Catalog</option><option value="frozen-definitions">Frozen definitions</option></select><div class="coverage-legend"><span class="coverage-status healthy">Healthy</span><span class="coverage-status attention">Attention</span><span class="coverage-status degraded">Degraded</span><span class="coverage-status blind-spot">Blind spot</span></div></div></div><div class="coverage-table-wrap"><table class="coverage-table"><thead><tr><th>Observation / detector</th><th>Origin</th><th>Coverage</th><th>Gap</th><th>Rules</th><th>Producer revision</th><th>State</th></tr></thead><tbody id="coverageRows"><tr><td colspan="7" class="workspace-empty">Loading detection coverage…</td></tr></tbody></table></div><div id="coverageDetail" class="detail"><div class="workspace-empty">Select an observation family to inspect its producer, dependent rules and current-version coverage gaps.</div></div></section></section><section id="workbench-threat-intelligence" class="workspace-view" data-workbench-view="threat-intelligence"><div class="workspace-heading"><div><h1>Threat Intelligence</h1><p>Review every URL/host Omega currently observes, the public IPs resolved in the frozen daily snapshot, and whether those endpoints match active threat intelligence. An unlisted endpoint is not a safe verdict.</p></div><span class="badge ro">FROZEN DAILY · READ ONLY</span></div><div id="threatIntelCards" class="cards workspace-cards"></div><section class="panel"><div class="panelhead"><div><h2>Current endpoint inventory</h2><div class="muted small">Current plugin versions only. DNS is resolved by the daily collector; SRL evaluates only frozen feed-backed matches, never a live reputation lookup during plugin analysis.</div></div><span id="threatIntelRevision" class="muted small"></span></div><div class="detail"><div class="coverage-toolbar"><input id="threatIntelSearch" placeholder="Filter host, URL, IP, category or source…"><select id="threatIntelStateFilter"><option value="">All endpoints</option><option value="matched">Threat-intel matches</option><option value="unlisted">Unlisted</option><option value="resolution">DNS attention</option></select></div></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Host / URL</th><th>Resolved IPs</th><th>Risk / state</th><th>Category / source</th><th>Current plugins</th></tr></thead><tbody id="threatIntelMatchRows"><tr><td colspan="5" class="workspace-empty">Loading frozen threat intelligence…</td></tr></tbody></table></div></section><section class="panel" style="margin-top:12px"><details open><summary>Feed health & provenance</summary><div id="threatIntelFeeds" class="detail"></div></details><details><summary>Frozen indicator list</summary><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Indicator</th><th>Type</th><th>Risk</th><th>Category</th><th>Source</th><th>Last seen</th></tr></thead><tbody id="threatIntelIndicatorRows"></tbody></table></div></details><div id="threatIntelNote" class="muted small" style="padding:10px 14px"></div></section></section><section id="workbench-queue" class="workspace-view" data-workbench-view="queue"><div class="workspace-heading"><div><h1>Scan Queue</h1><p>Why SigmaScope is scanning what it is scanning, which coverage lane runs first, and whether a catalog rebuild actually invalidated prior coverage.</p></div><span class="badge ro">SIGMASCOPE QUEUE · READ ONLY</span></div><div id="queueCards" class="cards workspace-cards"></div><section class="panel"><div class="panelhead"><div><h2 id="queueHeadline">Queue causality</h2><div class="muted small">Explanation of the published queue state and deterministic selection contract.</div></div><span id="queuePolicy" class="muted"></span></div><div id="queueExplanation" class="detail"></div></section><div class="research-grid" style="margin-top:12px"><section class="panel"><div class="panelhead"><div><h2>Coverage lanes</h2><div class="muted small">Lower-numbered lanes are exhausted before later lanes.</div></div></div><div id="queueLanes" class="detail"></div></section><section class="panel"><div class="panelhead"><div><h2>Why work is queued</h2><div class="muted small">Primary invalidation/recheck reason for pending work.</div></div></div><div id="queueReasons" class="detail"></div></section></div><section class="panel" style="margin-top:12px"><div class="panelhead"><div><h2>Next selected work</h2><div class="muted small">Exact preview only when DeltaScope recognizes the published selection policy.</div></div><span id="queueOrderStatus" class="muted"></span></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>#</th><th>Plugin</th><th>Lane</th><th>Work</th><th>Reason</th><th>Priority</th><th>Attempts</th></tr></thead><tbody id="queueNextRows"><tr><td colspan="7" class="workspace-empty">Loading published queue…</td></tr></tbody></table></div></section></section><section id="workbench-reports" class="workspace-view" data-workbench-view="reports"><div class="workspace-heading"><div><h1>Reports</h1><p>Coverage, queue, replay and reanalysis readiness derived from published evidence. Reports are generated views, not stored mutable records.</p></div><span class="badge ro">READ ONLY</span></div><div id="reportCards" class="cards workspace-cards"></div><section class="panel"><div class="panelhead"><div><h2>Operational security reports</h2><div class="muted small">Coverage, queue and SRL cutover-readiness summaries.</div></div><span id="reportRevision" class="muted"></span></div><div id="reportRows" class="detail"></div></section><details id="metricsDrawer" class="advanced metrics-drawer" open><summary>Metrics & coverage · exact drill-down counts</summary><div id="allMetricCards" class="cards"></div></details></section>
...
const sev=s=>`sev-${String(s||'none').toLowerCase()}`;const fmt=n=>Number(n||0).toLocaleString();let timer;let tables=[];let currentTable=null;let currentRows=[];let currentFkLinks=[];let currentSummary=null;let currentPluginDetail=null;let currentInvestigatorCases=null;let currentInvestigatorCase=null;let currentInvestigatorCasework=null;let activeInvestigatorCaseId='';let currentMetric=null;let currentAssetRows=[];let currentWorkbenchView='developer';let currentWorkbenchCase=null;let currentRuleCatalog=null;let currentRuleLibrary=null;let currentRuleWorkspace=null;let currentRuleLibrarySelection=null;let currentWorkspaceRule=null;let currentVisualGraph=null;let selectedVisualNode=null;let visualConnectMode=false;let visualConnectSource=null;let ruleWorkspaceDirty=false;let currentReports=null;let currentScanQueue=null;let currentSystemStatus=null;let currentOperations=null;let currentCollectors=null;let currentDetectionCoverage=null;let currentThreatIntelligence=null;let selectedCoverageCollection=null;let currentDocs=null;let currentDocId=null;let ruleUndoStack=[];let ruleRedoStack=[];let ruleHistoryCurrent='';let ruleHistoryApplying=false;let globalSearchTimer=null;let currentSubject=null;let currentPerspective='developer';let currentPerspectiveRoute='developer:';let currentProfileBuilderBase={};let currentProfileBuilderCapabilities=[];let currentProfileYaml='';let currentCompare=null;let currentAssetRelationGraph=null;let headerPluginRows=[];let currentPluginInventory=null;let currentSourceStatus=null;let notificationItems=new Map();let latestFindingsPayload=null;let pluginPickerFilter='';let ruleLibraryCollapsed=false;let publishedRefreshInFlight=false;let lastPublishedRefreshError='';
...
function wireFindingLineage(host){host.querySelectorAll('[data-lineage-collection]').forEach(x=>x.addEventListener('click',()=>openRuleCollection(x.dataset.lineageCollection)));host.querySelectorAll('[data-lineage-rule]').forEach(x=>x.addEventListener('click',async()=>{const rule=x.dataset.lineageRule,pack=x.dataset.lineagePack||'';$('helpDrawer').hidden=true;$('helpDrawer').classList.remove('lineage-drawer');setPerspective('researcher',{navigate:false});setWorkbenchView('rules');await loadRuleCatalog();if(pack&&ruleLibraryRule(pack,rule))await selectSystemRule(pack,rule);else await openRuleFromFinding(rule)}))}
async function openFindingLineage(variantId,findingId='',ruleId=''){const id=Number(variantId||currentSubject?.variantId||0);if(!id)return;const drawer=$('helpDrawer');drawer.classList.add('lineage-drawer');$('helpTitle').textContent='Finding lineage';$('helpBody').innerHTML='<div class=workspace-empty>Reconstructing collector → observation → rule → finding lineage…</div>';drawer.hidden=false;try{const q=new URLSearchParams({variant_id:String(id)});if(findingId)q.set('finding_id',findingId);if(ruleId)q.set('rule_id',ruleId);const p=await api('/api/workbench/finding-lineage?'+q.toString()),f=p.finding||{},identity=p.identity||{},replay=p.exactReplay?'<div class=lineage-replay-pass><b>Exact selector replay available</b><div class="small">Complete retained collections were sufficient to replay this Stigma-1 lineage.</div></div>':`<div class=lineage-replay-bounded><b>${p.origin==='sigmascope-static'?'Static projection lineage':'Bounded lineage'}</b><div class="small">${esc(p.replayReason||'DeltaScope did not claim exact selector replay.')}</div></div>`;$('helpTitle').textContent=f.title||f.findingId||'Finding lineage';$('helpBody').innerHTML=`<div class=lineage-summary><div class=lineage-summary-box><div class=eyebrow>${esc(p.origin==='stigma-1'?'Stigma-1 / SRL lineage':'SigmaScope static lineage')}</div><h3>${esc(f.title||f.findingId||'Security finding')}</h3><div class="muted small">${esc(identity.name||'plugin')} · ${esc(identity.version||'')} · current variant ${fmt(identity.variantId||id)}</div><p>${esc(f.description||'')}</p><div><span class="pill ${sev(f.severity)}">${esc(f.severity||'none')}</span><span class=pill>${esc(f.ruleId||f.findingId||'')}</span></div></div><div>${replay}</div></div>${lineageFlowHtml(p)}<details><summary>Lineage relationships (${fmt(p.graph?.edges?.length||0)})</summary><div class=lineage-edge-table><table><thead><tr><th>From</th><th>Relationship</th><th>To</th></tr></thead><tbody>${(p.graph?.edges||[]).map(e=>`<tr><td><code>${esc(e.source||'')}</code></td><td>${esc(e.relationship||'')}</td><td><code>${esc(e.target||'')}</code></td></tr>`).join('')}</tbody></table></div></details><details><summary>Exact lineage projection</summary>${evidence({lineageProjectionId:p.lineageProjectionId,currentVersionOnly:p.currentVersionOnly,collections:p.collections,collectionCoverage:p.collectionCoverage,rules:p.rules,projection:p.projection})}</details><div class="muted small" style="margin-top:10px">Finding lineage is a read-only explanation over retained current-version evidence. It cannot change security conclusions or production policy.</div>`;wireFindingLineage($('helpBody'))}catch(e){$('helpBody').innerHTML=`<div class="rule-lab-diag error"><b>Could not reconstruct finding lineage</b><div>${esc(e.message)}</div></div>`}}
async function openRuleCollection(name){const collection=String(name||'').trim();if(!collection)return;const drawer=$('helpDrawer');drawer.classList.remove('lineage-drawer');$('helpTitle').textContent=collection;$('helpBody').innerHTML='<div class=workspace-empty>Loading collection contract and retained rows…</div>';drawer.hidden=false;try{const variant=Number(currentSubject?.variantId||0),payload=await api('/api/rule-lab/collection?name='+encodeURIComponent(collection)+'&variant_id='+encodeURIComponent(variant)+'&limit=100');const fields=payload?.fields&&typeof payload.fields==='object'?payload.fields:{};const identity=payload?.identity||{},plugin=identity.canonical_name||identity.canonicalName||identity.name||identity.internal_name||identity.internalName||'';const version=identity.assembly_version||identity.version||'';const current=variant?`<div class=collection-current><b>Selected plugin:</b> ${esc(plugin||('variant '+variant))}${version?` · ${esc(version)}`:''}<div class="small">The rows below are retained observations from the current selected version. Archive versions do not feed current rule totals.</div></div>`:`<div class=collection-empty><b>No plugin selected.</b><div class="small">This page still shows the collection contract. Use the plugin icon in the top bar to inspect the actual retained rows for a plugin.</div></div>`;$('helpBody').innerHTML=`${current}<div class=collection-meta><b>Collection</b><span><code>${esc(payload.collection||collection)}</code></span><b>Producer / source</b><span>${esc(payload.source||'not documented')}</span><b>Backing dataset</b><span><code>${esc(payload.dataset||'')}</code></span><b>Scope</b><span>${esc(payload.scope||'')}</span><b>Observation schema</b><span><code>${esc(payload.observationSchema||'')}</code></span><b>Completeness</b><span>${esc(payload.completeness||'unknown')}</span></div>${payload.notes?`<p>${esc(payload.notes)}</p>`:''}<h4>Typed fields available to Stigma-1</h4><table class=collection-fields><thead><tr><th>Field</th><th>Type</th></tr></thead><tbody>${Object.entries(fields).map(([field,type])=>`<tr><td><code>${esc(field)}</code></td><td><code>${esc(type)}</code></td></tr>`).join('')}</tbody></table><h4>Retained rows for selected plugin</h4>${variant?renderRuleCollectionRows(payload):''}<div class="muted small" style="margin-top:12px">This is a read-only observation view. Collection rows are inputs to deterministic rules; they are not findings by themselves.</div>`}catch(e){$('helpBody').innerHTML=`<div class="rule-lab-diag error"><b>Could not inspect ${esc(collection)}</b><div>${esc(e.message)}</div></div>`}}
...
const perspectiveConfig={
 ...
 investigator:{label:'Investigator',description:'Start from a suspicious or interesting plugin, keep a local case notebook, and follow findings, observations and relationships without losing the subject.',defaultView:'notebooks',note:'Published evidence stays read-only. Your case notes and pins are local analyst state only and never become security authority.',groups:[
  {label:'Investigate',items:[{label:'Cases',mark:'I',view:'notebooks'},{label:'Published findings',mark:'F',view:'incidents'},{label:'Plugins',mark:'P',view:'assets',tab:'overview'},{label:'Journey',mark:'J',view:'assets',tab:'journey'},{label:'Findings',mark:'F',view:'assets',tab:'findings'},{label:'Events',mark:'E',view:'events'},{label:'Relationships',mark:'R',view:'assets',tab:'relationships'},{label:'Intelligence',mark:'N',view:'intelligence'}]},
 ...
 operations:{label:'Operations',description:'Watch coverage, queues, publication health, workflow activity and production authority gates.',defaultView:'dashboard',note:'Operations is inspection-only in DeltaScope. Production queues, gates and workflows are controlled outside this workbench.',groups:[
  {label:'Operate',items:[{label:'Overview',mark:'D',view:'dashboard'},{label:'Pipelines',mark:'E',view:'events'},{label:'Collectors',mark:'K',view:'collectors'},{label:'Scan Queue',mark:'Q',view:'queue'},{label:'Evidence',mark:'V',view:'system'},{label:'Definitions & Gates',mark:'S',view:'system'}]},
...
function setWorkbenchView(name){currentWorkbenchView=name;document.querySelectorAll('[data-workbench-view]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchView===name));document.querySelectorAll('[data-workbench-nav]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchNav===name));const messages={developer:'Plugin Developer keeps your selected plugin at the center: review findings, explain expected behavior, improve source/build context, and generate a validated Omega profile.',dashboard:'Dashboard summarizes current security activity and component health.',notebooks:'Investigator cases are local notebooks for your own notes and evidence references. They do not become SigmaScope evidence, Stigma-1 facts, policy, or queue authority.',incidents:'Published findings starts with concrete findings and composes the read-only evidence around them.',events:'Events combines read-only workflow activity with normalized evidence events.',collectors:'Collectors shows what feeds Omega and whether acquisition quality is degrading over time: throughput, duration, freshness, failures and recent outputs.',queue:'Scan Queue explains why SigmaScope is scanning each item, whether first coverage or a real identity baseline is active, and why deterministic ordering can return to A.',coverage:'Detection Coverage audits SigmaScope itself: producer freshness, observation blind spots and the rules that depend on each collection.',
 'threat-intelligence':'Threat Intelligence shows every currently observed URL/host, its frozen DNS resolution, and any active feed match. UNLISTED means no feed match, not safe/clean.',intelligence:'Intelligence lets you pivot through shared endpoints, components and advisories.',assets:'Plugins is the object-centric dossier view. Select once, then follow the evidence without losing context.',rules:'Rules is the Stigma-1 workspace. System Rules remain read-only; My Rules are local and versioned.',reports:'Reports summarize coverage, queue and deterministic replay readiness.',docs:'Documentation is the deep reference; contextual explanations are also available beside evidence.',system:'System health answers whether evidence and components are healthy; exact revision IDs are advanced detail.'};toniSay(messages[name]||'DeltaScope remains read only.');if(name==='developer')renderDeveloperHome(currentPluginDetail);if(name==='rules')loadRuleCatalog();if(name==='reports')loadReports();if(name==='queue')loadScanQueue();if(name==='docs')loadDocs();if(name==='system'||name==='dashboard')loadSystemStatus();if(name==='dashboard'||name==='events')loadOperations(false);if(name==='collectors')loadCollectors(false);if(name==='coverage')loadDetectionCoverage(false);if(name==='threat-intelligence')loadThreatIntelligence(false);if(name==='notebooks')loadInvestigatorCases();if(name==='incidents')loadLatestFindings();updatePerspectiveNavActive()}
...
function renderScanQueue(q){currentScanQueue=q;const c=q?.counts||{};$('queueCards').innerHTML=card('Pending work',c.pending||0)+card('First coverage',c.firstCoverage||0)+card('Coverage retries',c.firstCoverageRetry||0)+card('Covered refresh',c.coveredRefresh||0)+textCard('Baseline reset',q?.baselineSecurityRebuild?'YES':'NO');$('queueHeadline').textContent=q?.headline||'Queue causality';$('queuePolicy').textContent=q?.selectionPolicy||'unknown selection policy';const notes=Array.isArray(q?.notes)?q.notes:[];$('queueExplanation').innerHTML=`<div class="research-callout ${q?.baselineSecurityRebuild?'warn':'pass'}"><b>${esc(q?.headline||'Published queue')}</b><div>${esc(q?.explanation||'No queue explanation is available.')}</div></div>${notes.map(n=>`<div class="muted small" style="margin-top:7px">• ${esc(n)}</div>`).join('')}<div class="muted tiny" style="margin-top:9px">Identity epoch ${esc(q?.catalogIdentityEpoch||'—')} · queue seed ${esc(q?.queueSeedRevision||'—')} · current Evidence variants ${fmt(q?.currentEvidenceVariants||0)}</div>`;const lanes=Array.isArray(q?.lanes)?q.lanes:[];$('queueLanes').innerHTML=lanes.length?lanes.map(l=>`<div class="research-row"><div class="research-row-head"><b>${esc(l.label)}</b><span class=badge>${fmt(l.count||0)}</span></div><div class="muted small">${esc(l.description||'')}</div></div>`).join(''):'<div class=workspace-empty>Exact lane semantics are unavailable for this selection policy.</div>';const reasons=Array.isArray(q?.reasons)?q.reasons:[];$('queueReasons').innerHTML=reasons.length?reasons.map(queueReasonRow).join(''):'<div class=workspace-empty>No pending queue reasons.</div>';const next=Array.isArray(q?.nextItems)?q.nextItems:[];$('queueOrderStatus').textContent=q?.selectionOrderExact?'exact coverage-first-v1 preview':'order not claimed';$('queueNextRows').innerHTML=next.length?next.map((x,i)=>`<tr><td>${i+1}</td><td><b>${esc(x.name||x.internalName||'—')}</b><div class="muted tiny">${esc(x.internalName||'')} · variant ${fmt(x.variantId||0)}</div></td><td>${esc(x.laneLabel||'—')}</td><td>${esc(x.workType||'—')}</td><td><b>${esc(x.primaryReason||'—')}</b><div class="muted tiny">${esc((x.reasons||[]).join(', '))}</div></td><td>${fmt(x.priority||0)}</td><td>${fmt(x.attemptCount||0)}${x.nextEligibleAtUtc?`<div class="muted tiny">after ${esc(x.nextEligibleAtUtc)}</div>`:''}</td></tr>`).join(''):'<tr><td colspan="7" class="workspace-empty">No pending selectable work in the published queue.</td></tr>'}
async function loadScanQueue(force=false){if(currentScanQueue&&!force){renderScanQueue(currentScanQueue);return}try{renderScanQueue(await api('/api/workbench/scan-queue'))}catch(e){$('queueNextRows').innerHTML=`<tr><td colspan="7" class=research-error>${esc(e.message)}</td></tr>`;$('queueExplanation').innerHTML=`<div class=research-error>${esc(e.message)}</div>`}}
...
async function loadSystemStatus(refresh=false){if(currentSystemStatus&&!refresh)return currentSystemStatus;try{const s=await api('/api/workbench/system');renderSystemStatus(s);return s}catch(e){$('systemChecks').innerHTML=`<div class=research-error>${esc(e.message)}</div>`}}
...
function renderLatestFindings(p){const rows=Array.isArray(p?.findings)?p.findings:[];$('latestFindingCount').textContent=`${rows.length} newest`;$('latestFindingRows').innerHTML=rows.map(f=>`<tr class=click data-latest-finding-variant="${Number(f.variantId||0)}"><td>${esc(f.occurredAtUtc||'—')}</td><td><b>${esc(f.plugin||f.internalName||'')}</b><div class="muted small">${esc(f.version||'')} · ${esc(f.sourceName||'')}</div></td><td><b>${esc(f.title||f.findingId||'Security finding')}</b><div class="muted small">${esc(f.ruleId||f.findingId||'')}</div><button class=finding-lineage-button data-latest-lineage="${esc(f.findingId||'')}" data-latest-lineage-rule="${esc(f.ruleId||'')}">Trace lineage</button></td><td class="${sev(f.severity)}">${esc(f.severity||'none')}</td></tr>`).join('')||'<tr><td colspan=4 class=workspace-empty>No current findings are published.</td></tr>';$('latestFindingRows').querySelectorAll('[data-latest-finding-variant]').forEach(x=>x.addEventListener('click',e=>{if(e.target.closest('[data-latest-lineage]'))return;loadWorkbenchCase(Number(x.dataset.latestFindingVariant),'incidents')}));$('latestFindingRows').querySelectorAll('[data-latest-lineage]').forEach(x=>x.addEventListener('click',e=>{e.stopPropagation();const row=x.closest('[data-latest-finding-variant]');openFindingLineage(Number(row?.dataset.latestFindingVariant||0),x.dataset.latestLineage||'',x.dataset.latestLineageRule||'')}));syncCriticalFindingNotifications(p)}
...
function renderDetectionCoverage(payload){currentDetectionCoverage=payload;const s=payload?.summary||{},c=payload?.corpus||{},rows=Array.isArray(payload?.collections)?payload.collections:[];$('coverageCards').innerHTML=card('Current variants',c.currentVariants||0,{},`${fmt(c.completeCurrentScans||0)} complete current scans`)+card('Healthy systems',s.healthy||0,{},`${fmt(s.systems||0)} observation/detector rows`)+card('Blind spots',s.blindSpots||0,{},`${fmt(s.degraded||0)} degraded · ${fmt(s.attention||0)} attention`)+card('Artifact revision current',c.artifactRevisionCurrent||0,{},`${fmt(s.variantsNotAtCurrentArtifactRevision||0)} variant(s) need attention`)+card('Source revision current',c.sourceRevisionCurrent||0,{},`${fmt(c.sourceAttributableVariants||0)} attributable source variants`);$('coverageMethod').innerHTML=`<b>How to read this matrix</b><div class="small">${esc(payload?.coverageMethodNote||'')}</div><div class="small" style="margin-top:5px"><code>${esc(payload?.revisions?.observationContractRevision||'')}</code> · rule dependencies from ${esc(payload?.ruleDependencyAuthority||'unknown')}</div>`;if(!selectedCoverageCollection||!rows.some(x=>x.collection===selectedCoverageCollection)){const firstGap=rows.find(x=>x.status!=='healthy');selectedCoverageCollection=(firstGap||rows[0]||{}).collection||null}renderCoverageRows();renderCoverageDetail(selectedCoverageCollection)}
function threatIntelStateLabel(r){if(r?.matched)return String(r.risk||'none').toUpperCase();const state=String(r?.reputationState||'unlisted');if(state==='resolution-failed')return 'DNS FAILED';if(state==='no-public-address')return 'NO PUBLIC IP';if(state==='unlisted-retained-dns')return 'UNLISTED · RETAINED DNS';return 'UNLISTED'}
function filteredThreatIntelRows(){const rows=currentThreatIntelligence?.endpointInventory||currentThreatIntelligence?.endpointMatches||[],q=String($('threatIntelSearch')?.value||'').trim().toLowerCase(),state=String($('threatIntelStateFilter')?.value||'');return rows.filter(r=>{if(state==='matched'&&!r.matched)return false;if(state==='unlisted'&&(r.matched||!String(r.reputationState||'').startsWith('unlisted')))return false;if(state==='resolution'&&!['resolution-failed','no-public-address','unlisted-retained-dns'].includes(String(r.reputationState||'')))return false;if(!q)return true;return [r.host,...(r.urlSamples||[]),...(r.resolvedIps||[]),...(r.categories||[]),...(r.sources||[])].join(' ').toLowerCase().includes(q)})}
function renderThreatIntelEndpointRows(){const rows=filteredThreatIntelRows(),body=$('threatIntelMatchRows');if(!body)return;body.innerHTML=rows.length?rows.map(r=>{const state=threatIntelStateLabel(r),stateClass=r.matched?sev(r.risk):'info',context=r.matched?`${esc((r.categories||[]).join(', ')||'feed match')}<div class="muted small">${esc((r.sources||[]).join(', '))}</div>`:`<span class="muted">No active feed match</span><div class="muted small">DNS: ${esc(r.resolutionStatus||'unknown')}</div>`;return `<tr><td><b>${esc(r.host||'')}</b><div class="muted small">${esc((r.urlSamples||[])[0]||'')}</div></td><td>${(r.resolvedIps||[]).map(ip=>`<code>${esc(ip)}</code>`).join('<br>')||'—'}</td><td><span class="pill ${stateClass}">${esc(state)}</span>${!r.matched?'<div class="muted small">not a clean verdict</div>':''}</td><td>${context}</td><td>${fmt((r.variantIds||[]).length)}${(r.variantIds||[]).length?`<div class="small">${(r.variantIds||[]).slice(0,5).map(id=>`<button data-threat-variant="${Number(id)}">${Number(id)}</button>`).join(' ')}</div>`:''}</td></tr>`}).join(''):'<tr><td colspan="5" class="workspace-empty">No endpoints match the current filter.</td></tr>';body.querySelectorAll('[data-threat-variant]').forEach(x=>x.addEventListener('click',async()=>{const id=Number(x.dataset.threatVariant||0);if(id){setWorkbenchView('assets');await loadDetail(id,'network')}}))}
function renderThreatIntelligence(payload){currentThreatIntelligence=payload;const s=payload?.summary||{},indicators=payload?.indicators||[],feeds=payload?.feeds||[];$('threatIntelCards').innerHTML=card('Observed hosts',s.observedHosts||0,{},`${fmt(s.matchedHosts||0)} active feed match(es)`)+card('Unlisted',s.unlistedHosts||0,{},'no active feed match · not a safe verdict')+card('DNS attention',s.unresolvedHosts||0,{},`${fmt(s.retainedDnsHosts||0)} retained prior resolution(s)`)+card('Active feeds',s.activeFeeds||0,{},`${fmt(s.degradedFeeds||0)} degraded · ${fmt(s.indicators||0)} indicators`)+card('Critical matches',s.criticalHosts||0,{},`${fmt(s.highHosts||0)} high-risk host(s)`);$('threatIntelRevision').textContent=`${payload?.reputationRevision||'no reputation revision'}${payload?.generatedAtUtc?' · '+payload.generatedAtUtc:''}`;$('threatIntelNote').textContent=payload?.note||'';renderThreatIntelEndpointRows();$('threatIntelFeeds').innerHTML=feeds.length?feeds.map(f=>`<div class="compare-change"><span class="compare-direction ${String(f.status||'')==='complete'?'added':'changed'}">${esc(String(f.status||'unknown').toUpperCase())}</span><div><b>${esc(f.name||f.id||'feed')}</b><div class="muted small">${esc(f.source||'')} · ${fmt(f.records||0)} record(s)${f.required?' · required':' · supplemental'}</div>${f.reason?`<div class="muted small">${esc(f.reason)}</div>`:''}</div></div>`).join(''):'<div class="workspace-empty">No threat-intelligence feeds are frozen in this Definitions snapshot.</div>';$('threatIntelIndicatorRows').innerHTML=indicators.map(r=>`<tr><td><code>${esc(r.value||'')}</code></td><td>${esc(r.indicatorType||'')}</td><td><span class="pill ${sev(r.risk)}">${esc(r.risk||'none')}</span>${r.active===false?'<div class="muted small">inactive / retained reference</div>':''}</td><td>${esc(r.category||'')}</td><td>${esc(r.source||'')}</td><td>${esc(r.lastSeen||'—')}</td></tr>`).join('')}

async function loadThreatIntelligence(refresh=false){if(currentThreatIntelligence&&!refresh){renderThreatIntelligence(currentThreatIntelligence);return currentThreatIntelligence}try{const p=await api('/api/workbench/threat-intelligence');renderThreatIntelligence(p);return p}catch(e){$('threatIntelMatchRows').innerHTML=`<tr><td colspan="5" class="workspace-empty"><b>Could not load threat intelligence</b><div>${esc(e.message)}</div></td></tr>`;return null}}
...
function renderInvestigatorCaseList(){const host=$('investigatorCaseList');if(!host)return;host.innerHTML=investigatorCaseListHtml(currentInvestigatorCases||{});host.querySelectorAll('[data-investigator-case]').forEach(x=>x.addEventListener('click',()=>openInvestigatorCase(x.dataset.investigatorCase)));renderActiveInvestigatorCaseBadge()}
...
async function init(){
 ...
 toniOverview();if(v2)$('advancedSql').style.display='none';renderTableList();await Promise.all([loadPlugins(),loadPluginCatalog()]);renderHeaderPluginSelect();await loadRelationshipCatalog();loadLatestFindings();loadOperations(false);await loadSystemStatus(true);renderDashboardPlatformOverview();$('newInvestigatorCase')?.addEventListener('click',createInvestigatorCase);loadInvestigatorCases()
}
function applySourceStatus(s){currentSourceStatus=s||{};const source=$('sourceBadge'),revision=$('revisionBadge');if(source){source.textContent=s.mode==='online'?'Online · verified published snapshot':String(s.mode||'Local');source.title=s.baseUrl||s.cacheDirectory||''}if(revision)revision.textContent=s.currentRevision||'—';const b=$('refreshEvidence');if(b){b.style.display=(s.updateAvailable||lastPublishedRefreshError)?'inline-block':'none';b.disabled=publishedRefreshInFlight;b.textContent=publishedRefreshInFlight?'Refreshing published state…':lastPublishedRefreshError?'Retry published refresh':`Refresh now · ${String(s.remoteRevision||s.remoteDefinitionsRevision||'published update').slice(0,28)}`}if(s.error)upsertNotification({id:'source-error',level:'fail',rank:90,title:'Evidence source problem',detail:s.error,meta:s.baseUrl||s.cacheDirectory||''});else removeNotification('source-error');renderDashboardPlatformOverview()}
...
async function refreshEvidence(){return refreshPublishedState({automatic:false,detected:currentSourceStatus})}
...
function developerReviewPlanHtml(d){const p=d?.developerReviewPlan;if(!p||!Array.isArray(p.actions))return'';const rows=p.actions.length?p.actions.map(a=>`<div class="developer-review-action ${esc(a.priority||'context')}"><span class=developer-review-priority>${esc(a.priority||'context')}</span><div><b>${esc(a.title||'Review item')}</b><div class="muted small">${esc(a.reason||'')}</div></div><button data-review-tab="${esc(a.targetTab||'overview')}">Open ${esc(a.targetTab||'overview')}</button></div>`).join(''):`<div class="muted small">No compact evidence cue currently requires a specific follow-up. This is not a safety verdict; exact evidence remains authoritative.</div>`;return `<div class="developer-review-plan ${esc(p.state||'clear')}"><div class=developer-review-head><div><div class=logical-divergence-kicker>Next actions · deterministic · read only</div><h4>${esc(p.headline||'Developer review plan')}</h4><div class="muted small">Prioritized from the selected variant's retained evidence and logical-plugin context. Nothing here changes findings, queues, Definitions, or publication state.</div></div><span class=pill>${fmt(p.actionCount||0)} action${Number(p.actionCount||0)===1?'':'s'}</span></div><div class=developer-review-actions>${rows}</div></div>`}
...
function catalogOnlyPluginHtml(d,id){const i=d.identity||{},ctx=d.catalogContext||{},covered=Number(ctx.currentEvidenceVariantCount||0),total=Number(ctx.activeVariantCount||(ctx.activeVariantIds||[]).length||1);return `<div class=readonly-boundary><b>Selected catalog variant · no current Evidence-v2</b><div class=small>This logical plugin exists in Omega's current catalog, but variant ${fmt(id)} has no current published Evidence-v2 scan. ${covered?`${covered} sibling variant${covered===1?' has':'s have'} current evidence; inspect them below.`:'No active sibling currently has published evidence either.'} This is not a clean/security verdict.</div></div><div class=cards>${card('Catalog plugin',ctx.pluginId||i.plugin_id||'—')} ${card('Active variants',total)} ${card('Covered variants',`${covered}/${total}`)} ${card('Selected state','UNSCANNED')}</div><div class=research-grid><section class=research-box><h4>Logical identity</h4>${kv(i,[['internal_name','Internal name'],['canonical_name','Name'],['author','Selected variant author'],['assembly_version','Selected variant version'],['repo_url','Project repository']])}</section><section class=research-box><h4>Why it is in My Plugins</h4><p>My Plugins is sourced from the canonical catalog logical-plugin index, not only from plugins that already have security evidence.</p><p class=muted>Grouping authority: catalog <code>plugin_id</code>. Assembly name/version are supporting context and never merge different catalog plugin identities.</p></section></div>${logicalPluginVariantsHtml(d,id)}<div class=research-box style="margin-top:12px"><h4>What happens next?</h4><p>When SigmaScope publishes current evidence for this active variant, the logical My Plugins row stays singular and this variant changes from unscanned to covered without losing its source/version identity.</p></div>`}
...
init().then(()=>{checkEvidenceRevision();setInterval(checkEvidenceRevision,60000)}).catch(e=>{document.body.innerHTML='<pre class=fail>'+esc(e.stack||e.message)+'</pre>'});
</script></body></html>'''
