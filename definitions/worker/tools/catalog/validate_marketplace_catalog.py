...
def validate_bytes(descriptor_bytes: bytes, bundle: bytes, *, require_v2: bool = False) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="omega-marketplace-validate-") as td:
        ...
        try:
            ...
            if int(db.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='marketplace_security_current'").fetchone()[0]):
                raise RuntimeError("retired marketplace_security_current table must not be present")
            summary_rows = db.execute(
                "SELECT security_dependencies_json,security_dependency_total_count,"
                "security_known_advisory_count,security_risk_score FROM runtime_plugin_variants"
            )
            ...
            if meta.get("client_projection_mode") != "fresh-allowlist-v1":
                raise RuntimeError("marketplace database is not the current fresh allow-list projection")
