...
/* DeltaScope UI contract v3.8: self-contained Tailwind-inspired spacing, cards and TONI rail. */
...
<div class="app-shell"><aside class="workbench-nav" aria-label="DeltaScope workbench"><div class="workbench-nav-title">Security information</div><button class="active" data-workbench-nav="dashboard"><span class="nav-mark">D</span>Dashboard</button><button data-workbench-nav="incidents"><span class="nav-mark">I</span>Incidents</button><button data-workbench-nav="events"><span class="nav-mark">E</span>Events</button><button data-workbench-nav="intelligence"><span class="nav-mark">N</span>Intelligence</button><button data-workbench-nav="assets"><span class="nav-mark">A</span>Assets</button><button data-workbench-nav="rules"><span class="nav-mark">R</span>Rules</button><button data-workbench-nav="reports"><span class="nav-mark">P</span>Reports</button><button data-workbench-nav="system"><span class="nav-mark">S</span>System</button><div class="workbench-nav-note"><b>READ ONLY</b><br>Analysis and Rule Lab scratch state stay local. Authoritative changes go through GitHub review/CI.</div></aside><main>
...
<div id="summaryCards" class="cards focus-cards"></div><div id="dashboardActivity" class="panel dashboard-activity"><div class="panelhead"><div><h2>Current security activity</h2><div class="muted small">Read-only priority view over published current evidence.</div></div></div><div id="dashboardActivityRows" class="detail"></div></div></section>
<section id="workbench-incidents" class="workspace-view" data-workbench-view="incidents"><div class="workspace-heading"><div><h1>Incidents</h1><p>Derived investigation cases from elevated findings, failed analysis and known advisories. DeltaScope does not assign, close or mutate incidents.</p></div><span class="badge ro">DERIVED · READ ONLY</span></div><section class="panel"><div class="panelhead"><h2>Current cases requiring attention</h2><span id="incidentCount" class="muted"></span></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Asset</th><th>Priority</th><th>Findings</th><th>Intelligence</th><th>Last evidence</th></tr></thead><tbody id="incidentRows"></tbody></table></div></section><section id="incidentCasePanel" class="panel case-projection"><div class="workspace-empty">Select an incident to compose its contributing findings, observations, intelligence and reprojection relationships.</div></section></section>
<section id="workbench-events" class="workspace-view" data-workbench-view="events"><div class="workspace-heading"><div><h1>Events</h1><p>Time-oriented security activity. Select an asset event to lazily compose its normalized retained-observation, finding, intelligence and SRL-reprojection timeline.</p></div><span class="badge ro">READ ONLY</span></div><section class="panel"><div class="panelhead"><h2>Latest security events</h2><span id="eventCount" class="muted"></span></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Time</th><th>Asset</th><th>Event</th><th>Severity</th><th>Source</th></tr></thead><tbody id="eventRows"></tbody></table></div></section><section id="eventCasePanel" class="panel case-projection"><div class="workspace-empty">Select an event to open the normalized read-only evidence timeline for that asset.</div></section></section>
...
<section id="workbench-rules" class="workspace-view" data-workbench-view="rules"><div class="workspace-heading"><div><h1>Rules</h1><p>Active Definitions are authoritative and read-only here. Rule Lab is local scratch space; a change becomes authoritative only after the GitHub candidate/review/CI path.</p></div><span class="badge ro">NO DIRECT ACTIVATION</span></div><div class="readonly-boundary"><b>Authority boundary:</b> DeltaScope can compile, explain and replay candidates, but cannot save Definitions, enable/disable production rules or rewrite findings.</div><div id="ruleCatalogCards" class="cards workspace-cards"></div><div class="research-grid"><section class="panel"><div class="panelhead"><div><h2>Active Definition Packs</h2><div class="muted small">Exact frozen pack/review/fixture provenance published with Evidence-v2.</div></div><span id="rulePackCount" class="muted"></span></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Pack</th><th>Tier</th><th>Review</th><th>Fixtures</th></tr></thead><tbody id="rulePackRows"></tbody></table></div></section><section class="panel"><div class="panelhead"><div><h2>Active rules</h2><div class="muted small">Frozen compiled SRL rules, not development-branch YAML.</div></div><span id="activeRuleCount" class="muted"></span></div><div class="workbench-table-wrap"><table class="workbench-table"><thead><tr><th>Rule</th><th>Kind</th><th>Output</th><th>Revision</th></tr></thead><tbody id="activeRuleRows"></tbody></table></div></section></div><section id="ruleProvenancePanel" class="panel case-projection"><div class="workspace-empty">Select an active rule or pack to inspect its exact provenance, reviewer, fixture and compiled semantics.</div></section><details id="ruleLab" class="advanced rule-lab" open><summary>Rule Lab · local/experimental SRL candidate sandbox</summary><div><div class="rule-lab-note"><b>No production write-back.</b> Candidate YAML is compiled and evaluated as inert local data with the same bounded SRL engine used by Definition Packs. Promotion still requires the separate reviewed repository workflow.</div><div class="rule-lab-shell"><section class="rule-lab-editor smart"><div class="panelhead" style="padding:0 0 9px;border:0"><div><h3>Candidate YAML</h3><div class="muted small">Schema-aware SRL v1 editor with live linting, typed completion and symbol/context intelligence. Nothing here modifies the scanner, Definitions, or Evidence-v2.</div></div><span class="pill readonly-pill">LOCAL ONLY</span></div><div class="rule-smart-toolbar"><div class="left"><span class="editor-chip">SRL v1</span><span id="ruleEditorHealth" class="editor-chip busy">INITIALIZING</span><span id="ruleEditorScope" class="editor-chip">typed retained evidence</span></div><div class="right"><button id="ruleEditorComplete" title="Context completion · Ctrl/Cmd+Space">Complete</button><button id="ruleFormat" title="Canonical YAML format · Shift+Alt+F">Format</button><label class="rule-spell-toggle" title="Use the browser spell checker for prose fields while editing"><input id="ruleSpellcheck" type="checkbox">Prose spellcheck</label></div></div><div class="rule-smart-editor"><div id="ruleGutter" class="rule-editor-gutter">1</div><div class="rule-code-wrap"><pre id="ruleHighlight" aria-hidden="true"></pre><textarea id="ruleYaml" spellcheck="false" autocapitalize="off" autocomplete="off" autocorrect="off" placeholder="Paste omega.sigmascope.rule.v1 or ruleset YAML here…"></textarea><div id="ruleCompletionPopup" class="rule-completion-popup" hidden></div></div></div><div class="rule-editor-statusbar"><span id="ruleCursor">Ln 1, Col 1</span><span id="ruleEditorMetrics">0 B · 1 line</span><span id="ruleEditorRevision">not compiled</span><span class="push">Ctrl/Cmd+Space complete · Ctrl/Cmd+Enter validate · Shift+Alt+F format</span></div><div class="rule-intelligence-grid"><div class="rule-intel-panel"><div class="rule-intel-title">Context intelligence</div><div id="ruleContextHelp" class="rule-context-doc">Move the caret through a rule to inspect keys, collections, fields, operators and symbols.</div><div id="ruleSuggestions" class="rule-suggestion-chips"></div></div><div class="rule-intel-panel"><div class="rule-intel-title">Outline / symbols</div><div id="ruleOutline" class="rule-outline"></div></div><div class="rule-intel-panel rule-flow-panel"><div class="rule-intel-title">Rule flow</div><div id="ruleFlow" class="rule-outline"><span class="rule-editor-hint">Collections, selectors, facts and findings are connected here as the candidate becomes valid.</span></div></div></div><div id="ruleInlineDiagnostics" class="rule-inline-diagnostics"></div><div class="rule-lab-actions"><input id="ruleImport" type="file" accept=".yaml,.yml,text/yaml"><button id="ruleExample">Load example</button><button id="ruleCompile">Validate now</button><button id="ruleEvaluate">Dry-run selected plugin</button></div><div class="rule-lab-actions"><input id="ruleVariantIds" placeholder="Variant IDs for set replay, comma-separated"><input id="ruleReplayLimit" type="number" min="1" max="1000" value="250" style="max-width:110px"><button id="ruleReplaySet">Replay set</button><button id="ruleReplayCorpus">Replay corpus</button></div><div class="rule-lab-fixture"><h3>Candidate fixtures</h3><div class="muted small">GitHub candidates require both polarities: positive fixtures must collectively exercise every candidate rule; negative fixtures must match none. Both are revalidated by GitHub CI.</div><div class="rule-proposal-grid"><div><h4>Positive fixture</h4><textarea id="rulePositiveFixture" spellcheck="false" placeholder="Positive fixture YAML…"></textarea><div class="rule-lab-actions"><button id="ruleCreatePositiveFixture">Create from selected</button><button id="ruleTestPositiveFixture">Test positive</button></div></div><div><h4>Negative fixture</h4><textarea id="ruleNegativeFixture" spellcheck="false" placeholder="Negative fixture YAML…"></textarea><div class="rule-lab-actions"><button id="ruleCreateNegativeFixture">Create from selected</button><button id="ruleTestNegativeFixture">Test negative</button></div></div></div><h3>GitHub proposal</h3><div class="muted small">This only opens GitHub's normal issue form with query-string prefills. The proposal path uses no GitHub API write or repository credentials and never submits the issue itself.</div><div class="rule-proposal-grid" style="margin-top:8px"><input id="rulePackId" placeholder="Candidate pack ID"><input id="rulePackTitle" placeholder="Candidate pack title"><textarea id="ruleRationale" class="meta" placeholder="Rationale"></textarea><textarea id="ruleFalsePositives" class="meta" placeholder="False-positive expectations"></textarea><textarea id="ruleProvenance" class="meta" placeholder="External provenance / source"></textarea><div><input id="ruleLicense" value="MIT" placeholder="License"><input id="ruleExportNotes" style="margin-top:8px" placeholder="Optional local export notes"></div><div class="wide rule-lab-actions"><button id="ruleExport">Export candidate bundle</button><button id="ruleProposeGitHub">Propose on GitHub</button></div></div><div id="ruleProposalResult"></div></div></section><section class="rule-lab-output"><div class="panelhead" style="padding:0 0 9px;border:0"><div><h3>Diagnostics & explanation</h3><div class="muted small">Selector matches, exact retained evidence rows, baseline diff and replay status.</div></div></div><div id="ruleLabStatus" class="muted">Compile a candidate to begin.</div><div id="ruleLabResult"></div></section></div></div></details></section>
...
function setWorkbenchView(name){currentWorkbenchView=name;document.querySelectorAll('[data-workbench-view]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchView===name));document.querySelectorAll('[data-workbench-nav]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchNav===name));const messages={dashboard:'Dashboard shows coverage and the highest-priority current security activity.',incidents:'Incidents are derived investigation cases. DeltaScope does not assign, close, or mutate them.',events:'Events are normalized read-only security activity. Select one to load its retained-evidence timeline lazily.',intelligence:'Intelligence is enrichment and pivoting context, not an authority that rewrites findings.',assets:'Assets are plugins first, with artifact, source, component and endpoint evidence underneath.',rules:'Rules shows the exact frozen active Definitions lineage. Rule Lab remains local scratch space and GitHub remains the promotion boundary.',reports:'Reports summarize coverage, queue and SRL reprojection readiness from published evidence.',system:'System exposes exact revisions, activation gates and audit state without control-plane actions.'};toniSay(messages[name]||'DeltaScope remains read only.');if(name==='rules')loadRuleCatalog();if(name==='reports')loadReports();if(name==='system')loadSystemStatus();}
...
async function init(){
 currentRuleCatalog=null;currentReports=null;currentSystemStatus=null;
 ...
 toniOverview();if(v2)$('advancedSql').style.display='none';renderTableList();await loadPlugins();await loadRelationshipCatalog()
}
...
class AppHandler(BaseHTTPRequestHandler):
    inspector: Any
    server_version = "OmegaDeltaScope/3.8"
    ...
    try:
        ...
        if args.command in {"rule-schema", "observation-schema", "capabilities"}:
            ...
            if args.command == "capabilities":
                ...
                payload["warning"] = "SRL v1 and Definition Pack v1 freezing are implemented. Reviewed staticPatternMatches observation rules emit the five primitive facts used by the first two compound correlations, and retained Evidence-v2 replay is available locally. Production projection remains gated until a compatible 2.15 corpus replays cleanly and cutover is explicitly reviewed; workers must load only frozen compiled Definitions."
