...
class SigmaScopeLocalQueueWorkerContractTests(unittest.TestCase):
    def test_local_worker_uses_fast_forward_expected_parent_publication(self) -> None:
        ...
        self.assertIn("def _git_blob_sha256", text)
        self.assertIn('["git", "show", f"HEAD:{relative_path}"]', text)
        self.assertIn("hashlib.sha256(completed.stdout).hexdigest()", text)
        self.assertIn("def _scanner_worktree_dirty", text)
        self.assertIn('"git", "status", "--porcelain"', text)
        self.assertNotIn("_sha256_file(repo / rel)", text)
