...
function setWorkbenchView(name){currentWorkbenchView=name;document.querySelectorAll('[data-workbench-view]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchView===name));document.querySelectorAll('[data-workbench-nav]').forEach(x=>x.classList.toggle('active',x.dataset.workbenchNav===name));const messages={developer:'Plugin Developer keeps your selected plugin at the center: review findings, explain expected behavior, improve source/build context, and generate a validated Omega profile.',dashboard:'Dashboard summarizes current security activity and component health.',incidents:'Cases starts with concrete findings and composes the evidence around them.',events:'Events combines read-only workflow activity with normalized evidence events.',collectors:'Collectors shows what feeds Omega and whether acquisition quality is degrading over time: throughput, duration, freshness, failures and recent outputs.',coverage:'Detection Coverage audits SigmaScope itself: producer freshness, observation blind spots and the rules that depend on each collection.',
 'threat-intelligence':'Threat Intelligence shows every currently observed URL/host, its frozen DNS resolution, and any active feed match. UNLISTED means no feed match, not safe/clean.',intelligence:'Intelligence lets you pivot through shared endpoints, components and advisories.',assets:'Plugins is the object-centric dossier view. Select once, then follow the evidence without losing context.',rules:'Rules is the Stigma-1 workspace. System Rules remain read-only; My Rules are local and versioned.',reports:'Reports summarize coverage, queue and deterministic replay readiness.',docs:'Documentation is the deep reference; contextual explanations are also available beside evidence.',system:'System health answers whether evidence and components are healthy; exact revision IDs are advanced detail.'};toniSay(messages[name]||'DeltaScope remains read only.');if(name==='developer')renderDeveloperHome(currentPluginDetail);if(name==='rules')loadRuleCatalog();if(name==='reports')loadReports();if(name==='docs')loadDocs();if(name==='system'||name==='dashboard')loadSystemStatus();if(name==='dashboard'||name==='events')loadOperations(false);if(name==='collectors')loadCollectors(false);if(name==='coverage')loadDetectionCoverage(false);if(name==='threat-intelligence')loadThreatIntelligence(false);if(name==='incidents')loadLatestFindings();updatePerspectiveNavActive()}
...
async function init(){
 ...
 toniOverview();if(v2)$('advancedSql').style.display='none';renderTableList();await loadPlugins();renderHeaderPluginSelect();await loadRelationshipCatalog();loadLatestFindings();loadOperations(false);await loadSystemStatus(true);renderDashboardPlatformOverview()
}
...
class AppHandler(BaseHTTPRequestHandler):
    inspector: Any
    server_version = "OmegaDeltaScope/4.18.0"
