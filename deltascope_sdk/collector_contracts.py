...
_BUNDLED_OBSERVATION_TYPES = OBSERVATION_TYPES
_BUNDLED_COLLECTORS = COLLECTORS


def configure_registry(document: Mapping[str, Any] | None) -> None:
    """Use a verified published collector/provider registry for this SDK process."""
    global _RUNTIME_REGISTRY, OBSERVATION_TYPES, COLLECTORS
    if document is None:
        _RUNTIME_REGISTRY = None
        OBSERVATION_TYPES = _BUNDLED_OBSERVATION_TYPES
        COLLECTORS = _BUNDLED_COLLECTORS
        return
    if not isinstance(document, Mapping) or str(document.get("schema") or "") != REGISTRY_SCHEMA:
        raise ValueError("unsupported published collector registry")
    observations = document.get("observationTypes")
    collectors = document.get("collectors")
    if not isinstance(observations, Mapping) or not isinstance(collectors, list):
        raise ValueError("published collector registry is incomplete")
    OBSERVATION_TYPES = {str(name): dict(spec) for name, spec in observations.items() if isinstance(spec, Mapping)}
    if len(OBSERVATION_TYPES) != len(observations):
        raise ValueError("published collector registry contains malformed observation types")
    COLLECTORS = tuple(dict(item) for item in collectors if isinstance(item, Mapping))
    if len(COLLECTORS) != len(collectors):
        raise ValueError("published collector registry contains malformed collectors")
