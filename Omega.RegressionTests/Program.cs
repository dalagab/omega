...
var tests = new (string Name, Action Body)[]
{
    ...
    ("normalized plugin dependency graph remains package-authoritative and IPC-safe", TestNormalizedPluginDependencyGraphContract),
    ("dependency resolver builds recursive leaf-first install plans", TestDependencyResolverRecursiveClosure),
    ("dependency resolver intersects constraints and reports conflicts", TestDependencyResolverConstraintIntersectionAndConflicts),
    ("dependency resolver fails closed for missing providers unknown grammar and cycles", TestDependencyResolverFailsClosedForMissingUnknownAndCycles),
    ("dependency resolver keeps optional and IPC relationships out of automatic closure", TestDependencyResolverOptionalAndIpcBoundary),
    ("dependency resolver respects compatibility and deterministic source preference", TestDependencyResolverCompatibilityAndDeterministicSourceSelection),
    ("dependency install plans freeze and invalidate catalog revisions", TestDependencyInstallPlanRevisionContract),
    ("dependency transaction executes safely with ownership rollback and autoremove guards", TestDependencyTransactionExecutionContract),
    ("repository migration prefers known and source-owner publishers before divergent mirrors", TestMigrationRepositoryProvenancePreferenceContract),
    ("SQLite bundle replacement contract stays strict", TestCatalogBundleImport),
    ("Definitions remains disk-backed and lazily hydrates verbose plugin data", TestLazyDefinitionsReadModelContract),
    ...
    ("startup manager stays one-shot, user-facing, and recovery-oriented", TestStartupHealthSnapshotContract),
    ...
    ("UI language stays concise and plugin HTML uses the shared renderer", TestConciseUiLanguageAndPluginMarkupContract),
    ...
    ("dependencies, repository states, and update review remain actionable", TestDependencyActionsRepositoryStateAndUpdateReviewContract),
    ("repository removal, collection state, and security provenance stay explicit", TestRepositoryRemovalCollectionsAndSecurityProvenanceContract),
