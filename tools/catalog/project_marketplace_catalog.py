...
def runtime_projection_digest(db: sqlite3.Connection) -> str:
    ...
    for row in cursor:
        digest.update(b"\n")
        digest.update(json.dumps(list(row), separators=(",", ":"), ensure_ascii=False, default=str).encode("utf-8"))
    return digest.hexdigest()
    ...
    db.execute(
        """CREATE VIEW runtime_plugin_variants AS
           SELECT
             ...
             COALESCE(w.description,'') AS website_description,COALESCE(w.image_urls_json,'[]') AS website_image_urls_json,
             ...
             COALESCE(sc.findings_json,'[]') AS security_findings_json,COALESCE(sc.source_available,0) AS security_source_available,
...
def project_database(evidence_database: Path, output_database: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="omega-marketplace-project-") as tmp:
        ...
        try:
            ...
            before_runtime = runtime_projection_digest(db)
            ...
            db.commit()
            after_runtime = runtime_projection_digest(db)
    ...
    with closing(sqlite3.connect(output_database)) as check:
        ...
        return {
            "integrity": "ok",
            "runtimeProjectionSha256": runtime_projection_digest(check),
