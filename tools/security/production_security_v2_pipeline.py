#!/usr/bin/env python3
"""Production Omega Security Evidence v2 scanner/orchestrator.

Security Evidence v2 is the authoritative scanner state.  The pipeline builds a
*temporary compact SQLite working projection* from the current catalog identities and
small v2 evidence needed by the existing scanner/projector, scans a bounded set of due
...
from local_security_v2_test_scanner import summarize as summarize_database  # noqa: E402
...
def materialize_current_state(base_database: Path, evidence: Path, work_database: Path) -> dict[str, Any]:
    """Build the bounded mutable state DB used by scanner/projector/audit.
    ...
    with closing(sqlite3.connect(temp)) as db:
        ...
        security_scan.refresh_dependency_graph(db, [])
        security_scan.recreate_runtime_view(db)
...
def _semantic_security_revision(db: sqlite3.Connection) -> str:
    ...
    safe_version = "".join(ch if ch.isalnum() or ch in ".-_" else "-" for ch in security_scan.SCANNER_VERSION)[:24]
...
def run_pipeline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scan_report = security_scan.run(scan_args)
    successful, failed = _restore_last_known_good(work_database, before_current)
    # The scanner itself preserves an existing completed current pointer when a
    ...
    refresh_args = _scanner_args(
        work_database,
        work_dir,
        report_name="security-advisory-refresh-report.json",
        max_scans=0,
        rescan_after_hours=args.rescan_after_hours,
        max_batch_seconds=0,
        internal_names=args.internal_names,
    ...
    refresh_report = security_scan.run(refresh_args)
    ...
    write_json(work_dir / "production-security-v2-report.json", result)
...
def self_test() -> None:
    ...
    # Full integration behavior is covered by tools/tests/test_production_security_v2_pipeline.py.
    assert security_scan.SCANNER_VERSION
    assert MAX_PUBLISH_FILE_BYTES >= 16 * 1024 * 1024
    print("Omega production Security Evidence v2 pipeline self-test passed.")
...
def main() -> int:
    ...
    if "--self-test" in sys.argv[1:]:
        self_test()
        return 0
    parser = argparse.ArgumentParser(description="Stage and validate the production Omega Security Evidence v2 update")
