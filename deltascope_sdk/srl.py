...
try:
    from . import observation_projection, rule_author_reference, deep_scan_contract, collector_contracts, component_registry, analysis_broker, capability_registry
...
def refresh_contracts(*, component_registry_document: Mapping[str, Any] | None = None, collector_registry_document: Mapping[str, Any] | None = None, capability_registry_document: dict[str, Any] | None = None) -> dict[str, Any]:
    """Refresh the local non-authoritative compiler vocabulary from verified published contracts."""
    global FIELD_REGISTRY
    if component_registry_document is not None:
        component_registry.configure_registry(component_registry_document)
    if collector_registry_document is not None:
        collector_contracts.configure_registry(collector_registry_document)
    if capability_registry_document is not None:
        capability_registry.configure_registry(capability_registry_document)
