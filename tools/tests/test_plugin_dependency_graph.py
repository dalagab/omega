...
class PluginDependencyGraphTests(unittest.TestCase):
    ...
    def test_build_graph_retains_unresolved_requirements_and_excludes_nonpackages(self) -> None:
        ...
        try:
            ...
            db.close()
        self.assertEqual(2, graph["counts"]["edges"])
        self.assertEqual(1, graph["counts"]["providers"])
        self.assertEqual(1, graph["counts"]["resolvedEdges"])
        self.assertEqual(1, graph["counts"]["unresolvedEdges"])
        self.assertEqual(2, graph["counts"]["requiredEdges"])
        self.assertEqual(1, graph["counts"]["blockedRequiredEdges"])
        self.assertEqual(1, graph["counts"]["installEligibleEdges"])
        ...
        self.assertNotIn("IpcProvider", edges)
