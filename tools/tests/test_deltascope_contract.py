...
class DeltaScopeContractTests(unittest.TestCase):
    def test_deltascope_is_explicit_and_read_only(self) -> None:
        ...
        self.assertIn("developer-only SigmaScope evidence and SRL Core workbench", wrapper)
        ...
        for token in ("Dashboard", "Incidents", "Events", "Intelligence", "Assets", "Rules", "Reports", "System", "data-workbench-nav", "data-workbench-view", "DERIVED · READ ONLY", "NO DIRECT ACTIVATION", "Authoritative changes go through GitHub review/CI", "DeltaScope does not assign, close or mutate incidents", "one SRL Core workspace"):
            self.assertIn(token, view)
