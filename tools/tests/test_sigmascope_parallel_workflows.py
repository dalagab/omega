...
class SigmascopeParallelWorkflowTests(unittest.TestCase):
    ...
    def test_shadow_orchestrator_has_no_schedule_or_publication_authority(self) -> None:
        ...
        self.assertIn("slot: [0, 1, 2, 3, 4, 5, 6, 7]", text)
        self.assertIn("max-parallel: 8", text)
        self.assertIn("actions/download-artifact@v8", text)
        self.assertIn("Resolve exact queue key for this static slot", text)
        self.assertIn("steps.assignment.outputs.active == 'true'", text)
        self.assertIn('image: ${{ needs.resolve-merge-image.outputs.image }}', text)
        self.assertIn('--queue-key "${{ steps.assignment.outputs.queue_key }}"', text)
        ...
        self.assertIn("needs: [plan, workers]", text)
