...
### DeltaScope
Developer/operator-only, read-only inspection and audit tooling over published or local SigmaScope evidence. DeltaScope 3.8 is organized as a **read-only security-information workbench**, not a database browser or administrative control plane: current variants enter a triage queue and open into Triage, Malware, Findings, Network, Code & native, Supply chain, and Immutable evidence tabs. It exposes ClamAV/YARA (including YARA archive-member targets), static findings/capabilities, automation/permission evidence, endpoint URLs/origin/confidence, imports/PInvoke/reachability/call search, dependency/native relationships, OSV, artifact/manifest identity, source-attribution/provenance and lifecycle/immutable analysis history. DeltaScope 3.8 also includes the local/experimental **Rule Lab**: inert SRL YAML import/edit, compile diagnostics, one-plugin retained-evidence dry runs, selector traces, baseline diffs, bounded set/corpus replay, fixture generation/testing, and deterministic candidate ZIP export. Rule Lab has no production write-back path. The Rule Lab editor is now schema-aware rather than a plain textarea: live safe-parser/compiler linting, line diagnostics, context-sensitive completion for SRL collections/fields/operators/condition symbols, code-keyword typo suggestions, syntax highlighting, rule/selector/fact/finding outline navigation, a live collection→selector→fact/finding flow view, canonical formatting, keyboard completion/validation shortcuts, and optional browser prose spellcheck all run against the same non-executable SRL v1 language contract. DeltaScope 3.8 completes the handoff with **Propose on GitHub**: after local candidate + positive/negative fixture validation it opens the normal GitHub Issue Form using query-string prefills; the operator reviews/submits in GitHub, and oversized URLs fall back to smaller prefills plus explicit copy actions rather than truncation. The proposal path performs no GitHub API write and needs no repository credentials. Incidents and Events now lazily open a selected read-only case projection that composes contributing findings/signals, advisory context, normalized retained-observation events and any Phase-10 SRL reprojection/reanalysis relationship before an investigator pivots into the full Asset view. Intelligence now also uses a bounded, published read-only relationship index to pivot endpoints, shared components and advisories across affected plugins; Assets adds a Relationships tab for plugin→variant→artifact/source→component/endpoint/advisory navigation. These relationships are context only (`policyInput=false`) and never become scanner/rule inputs. DeltaScope 3.8 additionally publishes and browses exact frozen Definition Pack/rule provenance from the authoritative Daily Definitions snapshot, including pack/rule revisions, review metadata, source hashes, fixtures and migration-parity state. Reports exposes coverage, SRL reprojection readiness and queue summaries; System exposes Evidence/Definitions/scanner/SRL revision lineage and hard read-only safety gates. Generic Evidence-v2/SQLite traversal remains available under Advanced. Online browsing detects publication races on the moving Evidence-v2 branch, refreshes the atomic root/index and retries while preserving strict SHA verification.
