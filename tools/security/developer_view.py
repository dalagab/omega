#!/usr/bin/env python3
"""DeltaScope: Omega security investigation and SRL Core developer workbench.
...
/* DeltaScope 4.0 unified SRL Core workspace. */
...
class AppHandler(BaseHTTPRequestHandler):
    inspector: Any
    server_version = "OmegaDeltaScope/4.0"
...
def serve(inspector: Any, host: str, port: int, open_browser: bool, rule_home: Path | None = None) -> int:
    rule_store = deltascope_rule_store.LocalRuleStore(rule_home)
    handler = type("BoundAppHandler", (AppHandler,), {"inspector": inspector, "rule_store": rule_store})
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return serve(inspector, args.host, args.port, not args.no_browser, args.rule_home)
