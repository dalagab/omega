#!/usr/bin/env python3
"""Local Omega security scanner for filling a disposable Security Evidence v2 test set.
...
Omega scanner then scans selected live plugin artifacts/sources into that working
...
Typical test run from the Omega repository root::
    ...
    python tools/security/local_security_v2_test_scanner.py \
      --evidence D:/OmegaEvidenceV2 \
      --work-dir D:/OmegaScannerTest \
      --max-scans 50

Scan every active downloadable variant (can take a long time)::
...
Set GITHUB_TOKEN or GH_TOKEN before running when source scanning is enabled.  A token
...
def prepare_work_database(source_database: Path, work_database: Path, *, reset: bool = False) -> dict[str, Any]:
    """Create/reuse the small mutable scanner database.

    Only catalog identity is cloned from the enormous v1 source.  Security tables are
    created fresh by security_scan.ensure_schema(), so local test scans don't copy the
    ...
    if source_database == work_database:
        raise RuntimeError("the local scanner work database must not be the v1 source database")
    ...
    if work_database.exists():
        with closing(sqlite3.connect(work_database)) as db:
            if not table_exists(db, "catalog_meta"):
                raise RuntimeError(f"existing work database is not an Omega local scanner DB: {work_database}")
            ...
            if row is None or str(row[0]) != marker:
                raise RuntimeError(
                    "existing local scanner database belongs to a different v1 source revision; "
    ...
    with closing(open_ro(source_database)) as source, closing(sqlite3.connect(temp)) as target:
        ...
        security_scan.ensure_schema(target)
        target.commit()
...
def run_local_scan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # security_scan currently reads GITHUB_TOKEN only.  Respect the common GH_TOKEN
    ...
    if not args.skip_source and not os.environ.get("GITHUB_TOKEN"):
        print(
            "WARNING: source scanning is enabled without GITHUB_TOKEN/GH_TOKEN; "
    ...
    first_args = _scanner_args(
        ...
        report_name="security-scan-report.json",
        max_batch_seconds=args.max_batch_seconds,
    )
    print(
        f"Local security scan: source={source_database.name}, eligible={total_eligible}, "
        f"requested={max_scans}, sourceScan={not args.skip_source}",
        flush=True,
    )
    scan_report = security_scan.run(first_args)
    ...
    refresh_args = _scanner_args(
        ...
        source_overrides=overrides,
        skip_source=True,
        report_name="security-projection-refresh-report.json",
        max_batch_seconds=0,
    )
    projection_report = security_scan.run(refresh_args)
    ...
    if not args.no_v2:
        print(f"Exporting fresh local scanner database to Security Evidence v2: {v2_output}", flush=True)
    ...
    write_json(work_dir / "local-security-test-report.json", result)
    return result
...
def self_test() -> None:
    ...
    # Verify the scanner-only fix that matters for real packaged plugins: exact
    ...
    security_scan.scan_dependency_json("FixturePlugin.deps.json", json.dumps(deps), intel)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with mock.patch.object(security_scan, "scan_row"):
    ...
    print("Omega local Security Evidence v2 test scanner self-test passed.")
...
def main() -> int:
    ...
    print(f"  Full report:             {Path(result['workDatabase']).parent / 'local-security-test-report.json'}")
    return 0
